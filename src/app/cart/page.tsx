          ...
          {activeStep === 1 ? (
            cart.map((item) => (
              ...
              <div className="flex items-center justify-between" key={item.id+item.selectedSize+item.selectedColor}>
                ...
                {/* DELETE BUTTON */}
                <button aria-label="Remove from cart" title="Remove" onClick={()=> removeFromCart(item)} className="w-8 h-8 rounded-full bg-red-100 hover:bg-red-200 transition-all duration-300 text-red-400 flex items-center justify-center cursor-pointer">
